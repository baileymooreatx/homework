#!/bin/bash
# Copied from https://packagecloud.io/imeyer/runit/install#bash-rpm
# Copying the script is safer than using curl because this fixed version has been reviewed.
# While this script could have been culled to be centos specific, it has not been edited.
unknown_os ()
{
...
{
  zypper --gpg-auto-import-keys refresh imeyer_runit
}
...
main ()
{
  ...
  yum_repo_config_url="https://packagecloud.io/install/repositories/imeyer/runit/config_file.repo?os=${os}&dist=${dist}&source=script"

  if [ "${os}" = "sles" ] || [ "${os}" = "opensuse" ]; then
    yum_repo_path=/etc/zypp/repos.d/imeyer_runit.repo
  else
